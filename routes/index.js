...
router.get('/category/*.:cat_id.html', function(req, res, next) {
  ...
  bookModel.find({ cat_id: category})
    .skip(pageOptions.page * pageOptions.limit)
    .limit(pageOptions.limit)
    .exec(function(err, dulieu) {
      bookModel.find({ cat_id: category}).count().exec(function (err, count) {
        var max_page=Math.floor(count/pageOptions.limit)-1;
...
router.get('/author/*.:id_author.html', function(req, res, next) {
  var author=req.params.id_author;
  ...
  bookModel.find({id_author: author},(error,data)=>{
    if(error){
      console.log(error)
    }else
    {
